...
export default class Image {
  ...
  public async link(image: string, width: number = 0, height: number = 0) {
    if (!isEmpty(image)) {
      ...
      if (width && height) {
        convertImage = path.resolve(
          DIR_IMAGE,
          "cache/" + width + "x" + height + "/" + image,
        );
      }
      ...
      if (!fs.existsSync(convertImage) && ext !== ".svg") {
        ...
        if (width !== 0 && height !== 0) {
          if (ext === ".png") {
            ...
            content.background(0xFFFFFFFF).contain(width, height).quality(90)
              .write(convertImage);
          }
        } else {
        ...
        await pluginEvent(
          "onImageResize",
          { width, height, image, convertImage },
        );
      } else if (ext === ".svg") {
      ...
      const output = await pluginEvent(
        "onImageResizeAfter",
        { width, height, imageUrl, ext, image },
      );
      if (output) {
        ({ width, height, imageUrl, ext, image } = output);
      }
      if (ext === ".svg") {
        return imageUrl + "static/images/" + path.dirname(image) + "/" +
          path.basename(image, ext) + ext;
      }
      if (width !== 0 && height !== 0) {
        return imageUrl + "static/images/cache/" + width + "x" + height + "/" +
          path.dirname(image) + "/" + path.basename(image, ext) + ext;
      } else {
        return imageUrl + "static/images/cache/" + path.dirname(image) + "/" +
          path.basename(image, ext) + ext;
      }
    }
