...
export default class Loader {
  ...
  public async model(route: string) {
    ...
    if (!this.registry.has('model_' + route.replace('/', '_'))) {
      let model = await import(pathToUri(Deno.cwd() + '/model/' + route+'.ts'))

      const modelName = 'Model' + route.split('/').map((value) => (capitalize(value))).join('')

      model = model[modelName]

      this.registry.set('model_' + route.replace('/', '_'), new model(this.registry))
    }
  }
