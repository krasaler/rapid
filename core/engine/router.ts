import { Application, Router as OakRouter, Context, RouterContext, send } from "https://deno.land/x/oak/mod.ts";
...
export default class Router {
  ...
  constructor() {
    ...
    this.app.use(mount(STATIC_BASE_URL + '/static', async (context: Context) => {
      await send(context, context.request.url.pathname, {
        root: DIR_STATIC
      });
    }))

    this.app.use(mount(STATIC_BASE_URL + '/stylesheet', async (context: Context) => {
      await send(context, context.request.url.pathname, {
        root: DIR_STYLESHEET
      });
    }))
  ...
  private async postRequest(ctx: RouterContext, next:  () => Promise<void>, route: any) {
    this.registry.set('response', new Response(ctx))
